...
using OnlinePvt.Gateway.Api.Shared.Calculations.CloudPoint;
...
namespace OnlinePVT.CSharp.FlashSample
{
    ...
    {
        public static async Task RunCloudPointSampleAsync(string? userId, string? accessSecret)
        {
            try
            {
        ...
        private static CloudPointCalculationInput CreateInput(OnlinePvtClient client) {
            var input = client.GetCloudPointInput();
            input.FluidId = new Guid("9E9ABAD5-C6CA-427F-B5E7-15AB3F7CF076");
            input.Temperature = 300;
            input.Pressure = 1;
            input.PointType = CloudPointCalculationType.FixedTemperature;
        ...
        private static void PrintCalculationResult(ApiOutputCalculationResultPoint result)
        {
            PrintLine();
            PrintValue("Property");
        ...
        private static void PrintComposition(ApiOutputCalculationResultPoint result)
        {
            PrintLine();
        ...
        private static void PrintProperties(ApiOutputCalculationResultPoint result)
        {
            var firstPhase = result.Phases[0];
            PrintLine();
